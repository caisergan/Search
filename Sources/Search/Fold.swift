        ...
        ZStack(alignment: .topLeading) {
            ...
            if folding, !prefs.sidebar, browser.peeking {
                ...
                TabBar(browser: browser)
                    .background {
                        // The desktop's glass on a glass theme, the window's
                        // ground on any other; the shadow falls from its edge.
                        Group {
                            if prefs.theme.isGlass { Backdrop(theme: prefs.theme) } else { Palette.ground }
                        }
